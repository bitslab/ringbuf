...
pub(crate) trait SavedBufferSpinMutexGuard<T> {
    fn futex_sleep(self);
    fn spin_on_condition(self, mutex: &Mutex<SavedBuffer<T>>) -> usize;
}

impl<T> SavedBufferSpinMutexGuard<T> for MutexGuard<'_, SavedBuffer<T>> {
    #[inline(always)]
    fn futex_sleep(self) {
    ...
    fn spin_on_condition(self, mutex: &Mutex<SavedBuffer<T>>) -> usize {
        ...
        loop {
            ...
            self_ref = mutex.spin_lock().unwrap();
        }
    }
}
